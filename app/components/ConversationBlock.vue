...
import type { MessageType } from '#shared/types/index.types'

const props = defineProps<MessageType>()
const emit = defineEmits<{
  (e: 'thump-up' | 'thumb-down', event: Event, id: string): void
}>()

const onThumbUp = (e: Event) => {
  emit('thump-up', e, props.id)
}
const onThumbDown = (e: Event) => {
  emit('thumb-down', e, props.id)
}
</script>

<template>
  <div>
    <div class="flex justify-end pl-md lg:pl-lg mb-sm">
      <div class="bg-light-4 text-dark-2 rounded-lg p-xs font-bold">
        {{ question }}
      </div>
    </div>
    <div class="flex justify-start pr-md lg:pr-lg mb-md">
      <div class="bg-dark-4 rounded-lg p-xs text-light-1">
        ...
        <div class="text-light-1 pt-3xs flex items-center gap-2">
          <button
            ...
            :class="[
              feedback === 'GOOD'
                ...
                : 'cursor-pointer',
              'p-3xs hover:bg-dark-2 rounded-md transition-colors duration-300 ease-linear',
            ]"
            @click="onThumbUp"
          >
            <svg
